...
- 已具备：
  - 异步评测任务：queued/running/done/failed，并支持 canceling/canceled（取消）
  ...
  - 前端：数据集/算法管理（本地持久化）、结果对比、快速选型（多指标加权）、CSV/Excel 导出；支持取消任务与展示失败/取消原因
  - Windows 本地一键启动脚本：`scripts/dev.cmd`（拉起 Redis/API/Worker/Web）
- 待完善（按资料摘录推进）：
  - 数据集/算法的存储与类型管理进一步完善（多任务、多类型、更真实数据验证）
  - 评测执行的鲁棒性：结构化错误码/重试策略、运行耗时/资源统计等
  - 报告导出更详细：包含执行时间、失败原因、配置快照（当前已支持导出 params_json 与 samples_json）
  - 视频任务（视频去噪/视频超分）的数据格式与评测流程落地

## 4.1 本地开发一键启动 (Dev Bootstrap)
- Windows（推荐）：在项目根目录执行 `.\scripts\dev.cmd`（默认执行 restart）
- 仅启动：`.\scripts\dev.cmd up`
