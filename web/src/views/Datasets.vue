<template>
  <div style="padding: 16px;">
    ...
    <div style="display:flex; gap:8px; margin-bottom: 12px;">
      ...
      <button @click="generateDemoOnBackend" style="padding:6px 10px;">生成后端 Demo 样例</button>
    </div>
...
async function generateDemoOnBackend() {
  const ok = confirm("将为 ds_demo 在后端生成 5 组样例（gt + 各任务输入目录）。继续吗？");
  if (!ok) return;
  ...
  try {
    ...
    alert(`已生成：${JSON.stringify(data.created)}`);
  } catch (e) {
