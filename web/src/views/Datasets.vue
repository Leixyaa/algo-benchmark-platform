<template>
  <div style="padding: 16px;">
    ...
    <div style="color:#666; margin-bottom: 12px;">
      管理评测数据集：新增、删除、查看元信息。支持导入 ZIP（gt/ + 各任务输入目录）。
    </div>

    <div style="display:flex; gap:8px; margin-bottom: 12px;">
      <button @click="openCreate" style="padding:6px 10px;">新增数据集</button>
      <button @click="chooseZipForNew" style="padding:6px 10px;">导入ZIP到选中数据集</button>
      <button
      ...
      <tbody>
        <tr v-for="ds in store.datasets" :key="ds.id">
          ...
          <td>
            ...
            <button @click="chooseZipFor(ds.id)" style="padding:4px 8px; margin-right:6px;">导入ZIP</button>
...
function _ensureFileInput() {
  ...
  input.addEventListener("change", async () => {
    ...
    const ok = confirm(`将导入 ZIP 到数据集：${dsId}${overwriteOnImport.value ? "（覆盖导入）" : ""}。继续吗？`);
    if (!ok) return;
    ...
    try {
      ...
      ElMessage({ type: "success", message: "导入成功，已刷新规模" });
    } catch (e) {
...
function chooseZipForNew() {
  if (!selectedDatasetId.value) return alert("请先选择导入目标数据集");
